(function () {
    ...
    function findLabel(target) {
        return Array.from(document.querySelectorAll('label')).find(function (el) {
            ...
            return target.labels.some(function (l) {
    ...
    function findSelectForLabel(label) {
        ...
        if (grandparent) {
            ...
            for (var j = parentIdx + 1; j < siblings.length; j++) {
                if (siblings[j].tagName === 'SELECT') return siblings[j];
                var deep = siblings[j].querySelector('select');
                if (deep) return deep;
                if (siblings[j].querySelector('label')) break;
            }
        }
        return null;
    }
    ...
    function buildWidget(label, placeholder) {
        ...
        function currentSelect() {
            ...
            if (!fresh || !fresh.isConnected) {
                var labelText = wrapper.dataset.scLabel;
                var cleaned = labelText.replace(/:$/, '');
                var newLabel = findLabel({ labels: [cleaned] });
                if (newLabel) {
                    label = newLabel;
            ...
            if (select && !select.isConnected) {
                var labelText2 = wrapper.dataset.scLabel;
                var cleaned2 = labelText2.replace(/:$/, '');
                var fallback = findLabel({ labels: [cleaned2] });
                if (fallback) {
                    label = fallback;
        ...
        function buildPanel(query) {
            ...
            try {
                ...
                if (!visibleItems.length) {
                    ...
                    if (!realOptions.length && loadingRetries < 10) {
                        loadingRetries++;
                        var delay = Math.min(500 * loadingRetries, 2000);
                        setTimeout(function () { if (open) buildPanel(q); }, delay);
                    }
                } else {
                    loadingRetries = 0;
                }
